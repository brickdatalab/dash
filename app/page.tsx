...
export default async function Page() {
  ...
  const [subWalletsR, snapshotsR, mirroredR] = await Promise.all([
    ...
    supa.from("dash_mirrored_trades").select("*").order("executed_at", { ascending: false }).limit(60),
  ]);

  const snapshots = (snapshotsR.data ?? []).map((s) => ({
    t: s.recorded_at as string,
    v: Number(s.balance_usd),
  }));
